...
class ApidaeLecture extends ApidaeCore
{
    ...
    public function __construct(array $params = null)
    {
        ...
        if (isset($params['debugQuery'])) {
            $this->debugQuery = $params['debugQuery'];
        }
        ...
        if (json_last_error() !== JSON_ERROR_NONE) {
            throw new ApidaeException('operations ko', ApidaeException::NO_JSON);
        }
        self::$queryOptions = json_decode(file_get_contents(realpath(dirname(__FILE__)) . '/queryOptions.json'), true);
        if (json_last_error() !== JSON_ERROR_NONE) {
            throw new ApidaeException('queryOptions ko', ApidaeException::NO_JSON);
        }
    }
    ...
    public function call($method, $params = null, $format = 'object')
    {
        ...
        if ($type_apidae == 'query') {
            ...
            if ($operation['parameters'] == 'queryOptions') {
                foreach (self::$queryOptions as $param => $desc) {
                    ...
                    if ($desc['type'] == 'array') {
                        if (!is_array($value)) {
                            $erreurs_param[] = $param . ' must be an array';
                        } else {
                    ...
                    if ($desc['type'] == 'int' && !is_int($value)) {
                        $erreurs_param[] = $param . ' must be an integer';
                    }
                    if ($desc['type'] == 'string' && !is_string($value)) {
                        $erreurs_param[] = $param . ' must be a string';
                    }

                    // Vérification du radius
                    if ($param == 'radius') {
                        if (!$this->isJson($value)) {
                            $erreurs_param[] = $param . ' must be a json value (ex:' . $desc['examples'] . ')';
                        }
                    }

                    if (sizeof($erreurs_param) > 0) {
                        $erreurs = array_merge($erreurs, $erreurs_param);
                    }
        ...
        if (isset($operation['parameters']) && is_array($operation['parameters'])) {
            foreach ($operation['parameters'] as $param => $desc) {
                ...
                if ($desc['type'] == 'array') {
                    if (!is_array($value)) {
                        $erreurs[] = $param . ' must be an array';
                    } else {
                        if ($desc['items']['type'] == 'int' && sizeof(array_filter($value, "is_int")) != sizeof($value)) {
                ...
                } elseif ($desc['location'] == 'query') {
                    $request_query[$param] = $value;
                } elseif ($desc['location'] == 'query_apidae') {
                    $apidae_query[$param] = $value;
                }
            }
        }
        ...
        try {
            ...
            if ($e->hasResponse()) {
                ...
                $responseBodyAsString = $response->getBody()->getContents(); // https://stackoverflow.com/a/30957410/2846837
                ...
                if ($this->isJson($responseBodyAsString)) {
                    $details = array_merge($details, json_decode($responseBodyAsString, true));
                } else {
                    $details['body'] = $responseBodyAsString;
                }
                throw new ApidaeException('API RequestException : ' . $code, ApidaeException::INVALID_HTTPCODE, $details);
            }
        }
